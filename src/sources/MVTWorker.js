    ...
    _decode (mvtLayer, metadata, mvtExtent, geometries, decodeFn) {
        ...
        for (let i = 0; i < mvtLayer.length; i++) {
            ...
            if (f.properties[metadata.idProperty] === undefined) {
                throw new CartoValidationError('mvt', `undefinedIdProperty[${metadata.idProperty}`);
            }
    ...
    decodeProperty (metadata, propertyName, propertyValue) {
        if (typeof propertyValue === 'string') {
            if (metadata.properties[propertyName].type !== 'category') {
                throw new CartoValidationError('mvt', `decodingPropertyErrorType[${propertyName}, ${metadata.properties[propertyName].type}, string, ${propertyValue}]`);
            }
            return metadata.categorizeString(propertyName, propertyValue);
        } else if (typeof propertyValue === 'number') {
            if (metadata.properties[propertyName].type !== 'number') {
                throw new CartoValidationError('mvt', `decodingPropertyErrorType[${propertyName}, ${metadata.properties[propertyName].type}, number, ${propertyValue}]`);
            }
