...
* @api
*/
export default class LinearEnd extends BaseExpression {
    constructor (input, min, max) {
        checkMaxArguments(arguments, 3, 'linearEnd');
        ...
        checkExpression('linearEnd', 'input', 0, input);
        checkExpression('linearEnd', 'min', 1, min);
        checkExpression('linearEnd', 'max', 2, max);
    ...
    eval (feature) {
        if (this.input.type === 'date') {
            ...
            const input = this.input.eval(feature).endValue;
    ...
    _bindMetadata (metadata) {
        ...
        if (this.input.type === 'date') {
            ...
            console.log('LINE',this.input.propertyName,min,max,smin,smax);
            ...
            this.inlineMaker = (inline) => `((${inline.input.end}-(${smin.toFixed(20)}))/(${(smax - smin).toFixed(20)}))`;
        } else {
            checkType('linearEnd', 'input', 0, 'number', this.input);
            checkType('linearEnd', 'min', 1, 'number', this.min);
            checkType('linearEnd', 'max', 2, 'number', this.max);
