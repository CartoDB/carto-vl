...
/**
 * Generates a list with all the features
 *
 ...
 *   variables: {
 *     list: s.globalFeatures(s.prop('value'), s.prop('category'))
 *   }
 * });
 ...
 * const source = carto.source.Dataset('data');
 * const viz = new carto.Viz(`
 *   \@list: globalFeatures($value, $category)
 * `);
 * const layer = carto.Layer('layer', source, viz);
 * ...
 * layer.on('updated', () => {
 ...
 * @api
 */
export default class GlobalFeatures extends BaseExpression {
    ...
    _applyToShaderSource () {
        throw new CartoRuntimeError('\'globalFeatures\' cannot be used in visualizations.');
    }
    ...
    _resetViewportAgg (metadata, renderLayer) {
        if (!this._FeatureProxy) {
            if (!this._requiredProperties.every(p => validProperty(p))) {
                throw new CartoValidationError(`${cvt.INCORRECT_TYPE} globalFeatures arguments can only be properties`);
            }
