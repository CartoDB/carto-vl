...
export default class Buckets extends BaseExpression {
    constructor (input, list) {
        ...
        if (input.type) {
            if (input.type !== 'number' && input.type !== 'category') {
                throw new CartoValidationError('expressions', `bucketsInvalidInputType[${input.type}]`);
            }
        ...
        if (list.elems) {
            list.elems.map((item, index) => {
                if (item.type) {
                    const ordinal = getOrdinalFromIndex(index + 1);
                    if (looseType && looseType !== item.type) {
                        throw new CartoValidationError('expressions', `bucketsInvalidParamTypeWithExpected[${ordinal}, ${looseType}, ${item.type}]`);
                    } else if (item.type !== 'number' && item.type !== 'category') {
                        throw new CartoValidationError('expressions', `bucketsInvalidParamType[${ordinal}, ${item.type}]`);
                    }
                }
    ...
    _bindMetadata (metadata) {
        ...
        if (this.input.type !== 'number' && this.input.type !== 'category') {
            throw new CartoValidationError('expressions', `bucketsInvalidInputType[${this.input.type}]`);
        }
        ...
        this.list.elems.map((item, index) => {
            ...
            if (this.input.type !== item.type) {
                throw new CartoValidationError('expressions', `bucketsInvalidParamTypeWithExpected[${ordinal}, ${this.input.type}, ${item.type}]`);
            } else if (item.type !== 'number' && item.type !== 'category') {
                throw new CartoValidationError('expressions', `bucketsInvalidParamType[${ordinal}, ${item.type}]`);
            }
        });
