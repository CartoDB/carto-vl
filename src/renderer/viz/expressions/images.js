 ...
 *   symbol: s.ramp(s.buckets(s.prop('pop_max'), [10]), s.images([s.image('./marker.svg'), s.image('./marker2.svg')]))
 * });
 *
 * @example <caption>Match different images to the different categories generated by buckets. (String)</caption>
 * const viz = new carto.Viz(`
 *    symbol: ramp(buckets($pop_max, [10]), images([image('./marker.svg'), image('./marker2.svg')]))
 * `);
 *
...
export default class Images extends Base {
    constructor(images) {
        checkArray('images', 'images', 0, images);
        images.forEach((image, i) => checkLooseType('images', `images[${i}]`, 0, 'image', image));
        const children = {};
        images.forEach((image, i) => children[`image${i}`] = image);
        super(children);
