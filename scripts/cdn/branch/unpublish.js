/**
 * Unpublish packages from our CDN
 */
...
function deleteFiles (branch) {
    console.log('Unpublishing branch:', branch);

    let uploader = client.deleteDir({
        Bucket: secrets.AWS_S3_BUCKET,
        Prefix: 'carto-vl/' + branch + '/'
    });

    uploader.on('error', function (err) {
        console.error('Error: unable to delete:', err.stack);
    });
